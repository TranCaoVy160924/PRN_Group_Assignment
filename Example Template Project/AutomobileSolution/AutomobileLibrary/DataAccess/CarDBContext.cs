...
namespace AutomobileLibrary.DataAccess
{
    public class CarDBContext: BaseDAL
    {
        ...
        public IEnumerable<Car> GetCarList()
        {
            ...
            string SQLSelect = "Select CarID, CarName, " +
        ...
        public Car GetCarByID(int carID)
        {
            ...
            try
            {
                var param = dataProvider.CreateParameter(
                    "@CarID", 4, carID, DbType.Int32);
                dataReader = dataProvider.GetDataReader(
        ...
        public void AddNew(Car car)
        {
            try
            {
                ...
                if (pro == null)
                {
                    string SQLInsert = "Insert Cars values(" +
                        "@CarID, $CarName, @Manufacturer, " +
                        "@Price, @ReleaseYear)";
                    var parameters = new List<SqlParameter>();
                    parameters.Add(dataProvider.CreateParameter(
                        "@CarID", 4, car.CarID, DbType.Int32));
                    parameters.Add(dataProvider.CreateParameter(
                        "@CarName", 50, car.CarName, DbType.String));
                    parameters.Add(dataProvider.CreateParameter(
                        "@Manufacturer", 50, car.Manufacturer, 
                        DbType.String));
                    parameters.Add(dataProvider.CreateParameter(
                        "@Price", 50, car.Price, DbType.Decimal));
                    parameters.Add(dataProvider.CreateParameter(
                        "@ReleaseYear", 4, car.ReleaseYear, 
                        DbType.Int32));
                    dataProvider.Insert(SQLInsert, CommandType.Text,
                        parameters.ToArray());
                }
                else
                {
        ...
        public void Update(Car car)
        {
            try
            {
                ...
                if (pro == null)
                {
                    string SQLInsert = "Update Cars set " +
                        "CarName = @CarName, " +
                    ...
                    parameters.Add(dataProvider.CreateParameter(
                        "@ReleaseYear", 4, car.ReleaseYear,
                        DbType.Int32));
                    dataProvider.Update(SQLInsert, CommandType.Text,
                        parameters.ToArray());
                }
                else
                {
        ...
        public void Remove(int carID)
        {
            try
            {
                ...
                if (car != null)
                {
                    string SQLDelete = "Delete Cars " +
                        "where CarID = @CarID";
                    var param = dataProvider.CreateParameter(
                        "@CarID", 4, carID, DbType.Int32);
                    dataProvider.Delete(SQLDelete, 
